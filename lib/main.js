...
function gger( order, M, N, alpha, x, strideX, y, strideY, A, LDA ) {
	...
	if ( !isLayout( order ) ) {
		throw new TypeError( format( '2NPFx', order ) );
	}
	if ( M < 0 ) {
		throw new RangeError( format( '2NPGE', M ) );
	}
	if ( N < 0 ) {
		throw new RangeError( format( '2NPFz', N ) );
	}
	if ( strideX === 0 ) {
		throw new RangeError( format( '2NPGB', strideX ) );
	}
	if ( strideY === 0 ) {
		throw new RangeError( format( '2NPG0', strideY ) );
	}
	...
	if ( LDA < max( 1, vala ) ) {
		throw new RangeError( format( '2NPGC', vala, LDA ) );
	}
